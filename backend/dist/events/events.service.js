...
let EventsService = class EventsService {
    ...
    async getById(id) {
        try {
            ...
            if (event.teams.length > 0) {
                ...
                event.players = playerRows.map((player) => ({
            ...
            event.chat_messages = chatRows.map((chat) => ({
            ...
            event.content = contentRows.map((content) => ({
    ...
    async create(eventData) {
        ...
        try {
            ...
            const [creatorRows] = await connection.execute('SELECT creator_id FROM creators WHERE creator_id = ?', [eventData.creator_id]);
            const creatorCheck = await connection.query('SELECT creator_id FROM creators WHERE creator_id = ?', [eventData.creator_id]);
            if (eventData.team1_id && eventData.team2_id) {
                if (eventData.team1_id === eventData.team2_id) {
                    throw new Error('Teams must be different');
                }
                const teamCheck = await connection.query('SELECT team_id FROM team WHERE team_id IN (?, ?)', [eventData.team1_id, eventData.team2_id]);
